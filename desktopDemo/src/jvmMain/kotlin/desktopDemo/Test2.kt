...
private fun Outer(value: State<String>, valueInner: State<String>) {
    ...
    val nextState = remember(value.value) { mutableStateOf(value.value + "modified outer") }
...
private fun Inner(value: State<String>) {
    ...
    val nextState = remember(value.value) { mutableStateOf(value.value + "modified inner") }
    Text(text = nextState.value)
}
